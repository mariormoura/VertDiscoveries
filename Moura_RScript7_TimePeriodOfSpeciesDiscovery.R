...
# SUPPORTING SCRIPT 7: SENSITIVITY ANALYSIS ACROSS TIME
############################################################################################################################
# SUPPORTING SCRIPT 7: SENSITIVITY ANALYSIS ACROSS TIME

# Steps in this script:
# Species-level sensitivity analysis
#  1. Identify the best error distribution across different time periods.
#  2. Run the model averaging procedure using species described across diffrent time periods.
#  3. Get the average weighted coefficients across different time periods.
#  4. Get species-level predictions for all aft models and across different time periods.
...
# STEP 1 - IDENTIFY THE BEST ERROR DISTRIBUTION ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 1 - IDENTIFY THE BEST ERROR DISTRIBUTION ACROSS DIFFERENT TIME PERIODS
...
# Define the time periods under investigation:
TimePeriod<-c(1758, seq(from=1760, to=1970, by=10)) # the starting date of each time period

# Create a dataframe to store the best error distributions identified for each vertebrate group and time period:
Outputs<-as.data.frame(matrix(nrow=1, ncol=3))  
colnames(Outputs)<-(c("Class","TimePeriod", "ErrorDistribution_1st"))

# Run the Accelerated failure time (AFT) models:
for (i in 1:4){ # i number of vertebrate groups
  ...
  trait_data<-trait_data[trait_data$Class==levels(trait_data$Class)[i],]
  ...
  TimePeriod_outputs<-as.data.frame(matrix(nrow=length(TimePeriod), ncol=3))  
  colnames(TimePeriod_outputs)<-(c("Class","TimePeriod", "ErrorDistribution_1st"))
  
  # For each time period, run AFT null model (TimToEvente ~ 1) using different error distribution and extrac the BIC:
  for(j in 1:length(TimePeriod)){ # 23 different time periods
    
    # Filter the species data to include only species described after TimePeriod[j]:
    trait_data_subset<-trait_data[trait_data$YearOfDescription>TimePeriod[j],]
    ...
    # Store the results
    TimePeriod_outputs[j,1]<-paste0(unique(trait_data_subset$Class)) # taxonomic class
    TimePeriod_outputs[j,2]<-TimePeriod[j] # starting time period of the species data 
    TimePeriod_outputs[j,3]<-Model_fit[1,1] # the best error distribution selected
    print(j)
    
  } # end of j for loop across time periods
  
  Outputs<-rbind(Outputs, TimePeriod_outputs)
...
FinalOutputs %>%
  dplyr::filter(TimePeriod==1758) %>%
  group_by(Class)

# Check for the most commonly selected error distribution across time periods:
FinalOutputs %>%
...
# For bird data, the results using AFT models with Weibull error distribution were qualitatively similar to those using Gompertz error distribution.
# Thus, to allow cross group comparisons, we kept Gompertz family for all groups.

#####

# STEP 2 - RUN THE MODEL AVERAGING PROCEDURE USING SPECIES DESCRIBED ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 2 - RUN THE MODEL AVERAGING PROCEDURE USING SPECIES DESCRIBED ACROSS DIFFERENT TIME PERIODS
rm(list=ls())
...
for(v in 1:4){ # v-number of vertebrate groups
  ...
  for(j in 1:length(TimePeriod)){
    ...
    colnames(set_of_models)<-(c("LogBodySize", "LogTaxPerClade", "LogRangeSize", "LogAMT", "LogAPP",
                                ...
                                "Parameter1", "Parameter1_SE", "Parameter2", "Parameter2_SE", "FamilyError", "BIC", "TimePeriod", "Class"))
    colnames(set_of_models_L95)<-c("LogBodySize", "LogTaxPerClade", "LogRangeSize", "LogAMT", "LogAPP",  "LogTS", "LogPS", "LogElevM",
                                   "LogPopD", "LogTaxPerBiome", "LogRarity", "Parameter1", "Parameter2", "FamilyError", "BIC", "TimePeriod", "Class")
    ...
    set_of_models[,29]<-TimePeriod[j]
    set_of_models_L95[,16]<-TimePeriod[j]
    set_of_models_U95[,16]<-TimePeriod[j]
...
dir.create("SensitivityAnalysis/TimePeriodOfDiscovery", showWarnings=F)
fwrite(list_set_of_models, "SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime.csv")
fwrite(list_set_of_models_L95, "SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime_L95.csv")
fwrite(list_set_of_models_U95, "SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime_U95.csv")

#####

# STEP 3. GET THE AVERAGE WEIGHTED COEFFICIENTS ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 3. GET THE AVERAGE WEIGHTED COEFFICIENTS ACROSS DIFFERENT TIME PERIODS
rm(list=ls())

# Re-read files (if re-running particular code sections):
FinalOutput<-read.csv("SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime.csv")
...
for (v in 1:4){
  ...
  for (t in 1:length(TimePeriod)){
    ...
    # Filter one time period:
    set_of_models<-FinalOutput_vert[FinalOutput_vert$TimePeriod==paste0(TimePeriod[t]),]
...
write.csv(AvgWeightedOutput,"SensitivityAnalysis/TimePeriodOfDiscovery/AverageWeightedCoefsAcrossTime.csv", row.names=F, fileEncoding="UTF-8")

#####

# STEP 4 - GET SPECIES-LEVEL PREDICTIONS FOR ALL AFT MODELS AND ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 4 - GET SPECIES-LEVEL PREDICTIONS FOR ALL AFT MODELS AND ACROSS DIFFERENT TIME PERIODS
rm(list=ls())

# Load the table containg all model coefs:
ModelParameters<-read.csv("SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime.csv")
ModelParameters_L95<-read.csv("SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime_L95.csv")
ModelParameters_U95<-read.csv("SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime_U95.csv")
...
# Get the species-level discovery metrics for each AFT model and time period:
a<-Sys.time()
for(v in 1:4){ # v-number of vertebrate groups
  ...
  for(t in 1:length(TimePeriod)){
    ...
    # Filter the species data to include only species described after TimePeriod[t]:
    trait_data<-raw_trait_data[raw_trait_data$YearOfDescription>TimePeriod[t],]
    
    # Filter the ModelParameters object to include one vertebrate group and one time period:
    set_of_models<-ModelParameters[ModelParameters$Class==VertGroups[v] & ModelParameters$TimePeriod==TimePeriod[t],]
    set_of_models_L95<-ModelParameters_L95[ModelParameters_L95$Class==VertGroups[v] & ModelParameters$TimePeriod==TimePeriod[t],]
    set_of_models_U95<-ModelParameters_U95[ModelParameters_U95$Class==VertGroups[v] & ModelParameters$TimePeriod==TimePeriod[t],]
    ...
    #DiscProb_dt_L95<-DiscProb_dt # the DiscProb_dt_L95 data.table retains something weird from DiscProb_dt (it will go wrong, but IDKW)
    #DiscProb_dt_U95<-DiscProb_dt # same as above
    ...
    list1[[t]]<-DiscProb_dt
    list2[[t]]<-DiscProb_dt_L95
    list3[[t]]<-DiscProb_dt_U95
    list4[[t]]<-YearPred_dt
    
    }

  # Export the outputs
  save(list1, file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/DiscProb_dt_", VertGroups[v], ".RData")))
  save(list2, file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/DiscProb_dt_L95_", VertGroups[v], ".RData")))
  save(list3, file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/DiscProb_dt_U95_", VertGroups[v], ".RData")))
  save(list4, file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/YearPred_dt_", VertGroups[v], ".RData")))
...
# STEP 5 - GET SPECIES-LEVEL AVERAGE WEIGHTED DISCOVERY METRICS ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 5 - GET SPECIES-LEVEL AVERAGE WEIGHTED DISCOVERY METRICS ACROSS DIFFERENT TIME PERIODS
rm(list=ls())
...
# Define the time periods under investigation and available taxa:
TimePeriod<-c(1758, seq(from=1760, to=1970, by=10)) # the starting date of each time period
VertGroups<-c("Amphibia", "Reptilia", "Mammalia", "Aves")
...
for(v in 1:4){ # v-number of vertebrate groups
  ...
  for(t in 1:length(TimePeriod)){
    ...
    # Read the trait data for the vertebrate group v and time period t:
    trait_data<-raw_trait_data[raw_trait_data$YearOfDescription>TimePeriod[t],]
    
    # Read the data.table containg the BIC of each AFT model and filter the vertebrate group v and time period t:
    set_of_models<-as.data.frame(fread("SensitivityAnalysis/TimePeriodOfDiscovery/ModelParametersAcrossTime.csv", stringsAsFactors=TRUE, encoding="UTF-8"))
    set_of_models<-set_of_models[set_of_models$Class==VertGroups[v] & set_of_models$TimePeriod==TimePeriod[t],]
    
    # Load the RData object for the vertebrate group v and time period t:
    DiscProb_dt<-loadRData(file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/DiscProb_dt_", VertGroups[v], ".RData")))[[t]]
    DiscProb_dt_L95<-loadRData(file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/DiscProb_dt_L95_", VertGroups[v], ".RData")))[[t]]
    DiscProb_dt_U95<-loadRData(file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/DiscProb_dt_U95_", VertGroups[v], ".RData")))[[t]]
    YearPred_dt<-loadRData(file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/YearPred_dt_", VertGroups[v], ".RData")))[[t]]
    ...
    trait_data$TimePeriod<-TimePeriod[t]
    
    # Store the outputs for each vertebrate group:
    if(v==1){AmphList[[t]]<-trait_data}
    if(v==2){ReptList[[t]]<-trait_data}
    if(v==3){MammList[[t]]<-trait_data}
    if(v==4){AvesList[[t]]<-trait_data}
...
# Export the results:
save(AmphList, file="SensitivityAnalysis/TimePeriodOfDiscovery/SpeciesLevelEstimatesAcrossTime_Amphibia.RData")
save(ReptList, file="SensitivityAnalysis/TimePeriodOfDiscovery/SpeciesLevelEstimatesAcrossTime_Reptilia.RData")
save(MammList, file="SensitivityAnalysis/TimePeriodOfDiscovery/SpeciesLevelEstimatesAcrossTime_Mammalia.RData")
save(AvesList, file="SensitivityAnalysis/TimePeriodOfDiscovery/SpeciesLevelEstimatesAcrossTime_Aves.RData")

#####

# STEP 6 - TAXON-LEVEL ANALYSIS, COMPUTE THE PER-TAXON DISCOVERY METRICS ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 6 - TAXON-LEVEL ANALYSIS, COMPUTE THE PER-TAXON DISCOVERY METRICS ACROSS DIFFERENT TIME PERIODS
rm(list=ls())
setwd("DefineYourDirectory")
...
for(v in 1:4){ # v-number of vertebrate groups
  ...
  for(l in 1:2){ # l taxonomic ranks
    ...
    # Read the species-level data on the estimates of discovery probability and filter one VertGroups:
    taxon_rank_data<-fread("SpeciesLevelData.csv", stringsAsFactors=TRUE, encoding="UTF-8")
    taxon_rank_data<-taxon_rank_data[taxon_rank_data$Class==VertGroups[v],]
    
    # Set the level of taxonomic rank (higher-level grouping or family):
    if(l==1){
      taxon_rank_data<-taxon_rank_data[, .(Binomial, Order, Suborder)]
      names(taxon_rank_data)[3]<-"TaxonRank"
    }
    if(l==2){
      taxon_rank_data<-taxon_rank_data[, .(Binomial, Order, Family)]
      names(taxon_rank_data)[3]<-"TaxonRank"
    }
    
    # Compute discovery metrics across all time periods:
    for(t in 1:length(TimePeriod)){
      
      # Read the trait data for the vertebrate group v and time period t:
      trait_data<-loadRData(file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/SpeciesLevelEstimatesAcrossTime_", VertGroups[v], ".RData")))[[t]]
      trait_data<-merge(trait_data, taxon_rank_data, by='Binomial', all.x=TRUE, allow.cartesian=TRUE)
      trait_data<-droplevels(trait_data)
      
      # Compute the estimated 'PropUnknown' for each level of TaxonRank:
      vert_summary<-trait_data %>%
        dplyr::group_by(Class, Order, TaxonRank) %>% 
        dplyr::summarise(Richness=length(TaxonRank),
      ...
      # Register the time period and taxonomic rank level and store output in a list:
      vert_summary$TimePeriod<-TimePeriod[t]
      
      if(l==1){
        vert_summary$TaxonLevel<-"Suborder"
        list1[[t]]<-vert_summary
        }  
      
      if(l==2){
        vert_summary$TaxonLevel<-"Family"
        list2[[t]]<-vert_summary
      }
      
      rm(vert_summary, trait_data)
      
    } # end of t for loop across time periods
    
    } # end of for loop (taxonomic rank levels)
  
  # Store the results for each vertebrate group:
  list1<-data.table::rbindlist(list1)
  list2<-data.table::rbindlist(list2)
  VertList[[v]]<-rbind(list1, list2)
  
}

# Unlist all results for terrestrial vertebrates:
VertList<-data.table::rbindlist(VertList)
fwrite(VertList, "SensitivityAnalysis/TimePeriodOfDiscovery/TaxonLevelEstimatesAcrossTime.csv")

#####

# STEP 7 - ASSEMBLAGE-LEVEL ANALYSIS, COMPUTE THE PER-ASSEMBLAGE DISCOVERY METRICS ACROSS DIFFERENT TIME PERIODS
##########################################################################################################################
# STEP 7 - ASSEMBLAGE-LEVEL ANALYSIS, COMPUTE THE PER-ASSEMBLAGE DISCOVERY METRICS ACROSS DIFFERENT TIME PERIODS
rm(list=ls())
setwd("DefineYourDirectory")
...
# List of directories and files to read the observed and subsampled assemblage data:
file_list<-list()
...
a<-Sys.time()
for (v in 1:1){ # v-number of vertebrate groups
  
  # Create empty lists to store the assemblage-level estimates of discovery potential:
  AssemblageAcrossTime_220km<-list()
  AssemblageAcrossTime_440km<-list()
  AssemblageAcrossTime_880km<-list()
  Results_220km<-list()
  Results_440km<-list()
  Results_880km<-list()
  ...
  for (j in 1:3){ # j spatial resolutions (220, 440, 880km)
    ...
    for(t in 1:length(TimePeriod)){
      ...
      # Load species-level data for the selected time period and vertebrate group:
      trait_data<-loadRData(file.path(paste0("SensitivityAnalysis/TimePeriodOfDiscovery/SpeciesLevelEstimatesAcrossTime_", VertGroups[v], ".RData")))[[t]]
      ...
      assemblage_data<-full_assemblage_data[Binomial %in% as.character(trait_data$Binomial)] # remove species described before 't'
      ...
      for(k in 1:length(sub_list[[j]])){ # k = sub_list value giving the number of pseudoreplications
        ...
        ssam<-ssam[Binomial %in% as.character(trait_data$Binomial)] # remove species described before 't'
        ssam<-droplevels(ssam)
        
        # Join tables (subsampled assemblage data and trait data):
        assemblage_with_traits<-merge(ssam, trait_data, by='Binomial', all.x=TRUE, allow.cartesian=TRUE)
        ...
        # Register the subsampling level and store the results according to the subsampling level:
        avg_subsampled_assemblages$Subsampling<-sub_list[[j]][k]
        avg_subsampled_assemblages$SpatialScale<-spatial_scale[j]
        avg_subsampled_assemblages$TimePeriod<-TimePeriod[t]
      ...
      if(j==1){AssemblageAcrossTime_220km[[t]]<-rbind(data.table::rbindlist(Results_220km))}
      if(j==2){AssemblageAcrossTime_440km[[t]]<-rbind(data.table::rbindlist(Results_440km))}
      if(j==3){AssemblageAcrossTime_880km[[t]]<-rbind(data.table::rbindlist(Results_880km))}
      
      } # end of t for loop across time periods
    
    } # end of j for loop across spatial resolutions
  
  # Unlist the output to get one single (and gigantic) data.table with all subsampling levels, time periods, spatial resolutions:
  AssemblageAcrossTime_220km<-data.table::rbindlist(AssemblageAcrossTime_220km)
  AssemblageAcrossTime_440km<-data.table::rbindlist(AssemblageAcrossTime_440km)
  AssemblageAcrossTime_880km<-data.table::rbindlist(AssemblageAcrossTime_880km)
  VertList[[v]]<-rbind(AssemblageAcrossTime_220km, AssemblageAcrossTime_440km, AssemblageAcrossTime_880km)
  
} # end of v for loop across vertebrate groups
b<-Sys.time()
b-a

# Expot the results for each vertebrate group:
fwrite(VertList[[1]], "SensitivityAnalysis/TimePeriodOfDiscovery/AssemblageLevelEstimatesAcrossTime_Amphibia.csv")
fwrite(VertList[[2]], "SensitivityAnalysis/TimePeriodOfDiscovery/AssemblageLevelEstimatesAcrossTime_Reptilia.csv")
fwrite(VertList[[3]], "SensitivityAnalysis/TimePeriodOfDiscovery/AssemblageLevelEstimatesAcrossTime_Mammalia.csv")
fwrite(VertList[[4]], "SensitivityAnalysis/TimePeriodOfDiscovery/AssemblageLevelEstimatesAcrossTime_Aves.csv")